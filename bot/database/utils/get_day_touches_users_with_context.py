...
async def get_day_touches_users_with_context(
    messages_limit: int = 15
) -> List[Tuple[int, str, List[Dict[str, Any]]]]:
    """
    Возвращает список пользователей с включенным notify_day_touches и их историей сообщений.
    ...
    # 1. Получаем ID и имена пользователей, у которых notify_day_touches = True
    async with AsyncSessionLocal() as session:
        try:
            stmt_users = select(User.telegram_id, User.name).where(
                User.notify_day_touches.is_(True)
            )
            ...
            logger.error(f"Ошибка get_day_touches_users_with_context: {e}")
