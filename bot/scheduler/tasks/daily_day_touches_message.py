...
async def daily_day_touches_message():
    # 1. Получаем данные
    users_data = await get_day_touches_users_with_context()
    logger.info(f"Начало дневной рассылки. Найдено пользователей: {len(users_data)}")
...
async def process_single_user(user: dict):
    ...
    trigger_message = "(Начни разговор сам. Просто спроси и узнай — как сейчас чувствует себя пользователь, пиши коротко, без давления)"
