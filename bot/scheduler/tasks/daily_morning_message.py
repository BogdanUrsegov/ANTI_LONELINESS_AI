...
async def daily_morning_message():
    # 1. Получаем данные
    users_data = await get_morning_users_with_context()
    logger.info(f"Начало утренней рассылки. Найдено пользователей: {len(users_data)}")
    ...
    if errors_count:
        logger.warning(f"Утренняя рассылка завершена. Ошибок: {errors_count} из {len(users_data)}")
    else:
